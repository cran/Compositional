    ...
    if ( type == "S" ) {
        ...
        if (!rann) {
            ...
            for (i in 1:length(a)) {
                ...
                ela[i, ] <- Rfast::knn.cv(folds = folds, nfolds = nfolds,
                  y = ina, x = z, k = k, dist.type = apostasi,
        ...
        if ( graph )  filled.contour(a, k, ela, ylab = "k nearest-neighbours",
                    xlab = expression(paste(alpha, " values")), cex.lab = 1.2)
        opt <- max(ela)
        ...
        for (i in 1:length(a)) {
            ...
            for (vim in 1:nfolds) {
                ...
                aba <- as.vector(folds[[vim]])
                aba <- aba[aba > 0]
                g <- Compositional::ait.knn(z[aba, ], z[-aba, ], ina = ina2, a = NULL,
                     k = k, type = "NS", mesos = mesos, apostasi = apostasi)
                be <- g - id
                per[vim, , i] <- Rfast::colmeans(be == 0)
            }
        }
        ...
        runtime <- proc.time() - runtime
        if (graph)
            filled.contour( a, k, ela, col = grey(1:11/11), ylab = "k nearest-neighbours",
                            xlab = expression(paste(alpha, " values")), cex.lab = 1.2 )
        opt <- max(ela)
        confa <- as.vector( which(ela == opt, arr.ind = TRUE)[1, ] )
        performance <- opt
