...
james <- function(y1, y2, a = 0.05, R = 999, graph = FALSE) {
  ...
  ybar1 <- as.vector( Rfast::colmeans(y1) ) ## sample mean vector of the first sample
  ybar2 <- as.vector( Rfast::colmeans(y2) )  ## sample mean vector of the second sample
  ...
  if (R == 1) {
    ...
    for (i in 1:R) {
      ...
      xb1 <- x1[b1, ]     ;      xb2 <- x2[b2, ]
      db <- as.vector( Rfast::colmeans(xb1) ) - as.vector( Rfast::colmeans(xb2) )  ## difference of the two mean vectors
